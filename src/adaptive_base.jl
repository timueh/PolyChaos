# TODO: Overfitting: proper checking, not only check 2 iterations
...
# end
function adaptiveBasis(opFun::Function, modelFun::Function, pceFun::Function, pceModel; maxDegree::Int = 20, Nrec::Int = maxDegree+1, ϵ = 1e-10)
    error = Inf     # error of current PC expansion
    error_opt = Inf    # error for optimal solution
    deg = 1         # current degree
    badFit = false  # overfitting flag
...
# Compute leave-one out error of the PCE basaed on a regression approach
function computeError(olsModel::OLSModel)
    Y = olsModel.Y
    Φ = olsModel.Φ
    coefficients = olsModel.pceCoeffs
    error = looError(Y, Φ, coefficients)
end
